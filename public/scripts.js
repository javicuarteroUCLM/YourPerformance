document.addEventListener("DOMContentLoaded", () => {
  ...
  const graficoRendimiento = document
  ...
  function generarCamposCursos() {
    ...
    if (numCursos) {
      for (let i = 0; i < numCursos; i++) {
        ...
        cursoLabel.textContent = `Notas de las asignaturas del ${obtenerDescripcionCurso(
          i,
          numCursos
        )}:`;
        ...
        numAsignaturasLabel.textContent = `   - Nº de asignaturas del ${obtenerDescripcionCurso(
          i,
          numCursos
        )}:`;
        ...
        numAsignaturasInput.value = notasGuardadas[`curso${i + 1}`]
          ? notasGuardadas[`curso${i + 1}`].numAsignaturas
          : ""; // Mantener el valor
        ...
        if (numAsignaturasInput.value) {
          generarNotasAsignaturas(
            notasDiv,
            i,
            parseInt(numAsignaturasInput.value)
          );
        }
  ...
  function generarNotasAsignaturas(notasDiv, cursoIndex, numAsignaturas) {
    ...
    const notasExistentes = notasGuardadas[`curso${cursoIndex + 1}`]
      ? notasGuardadas[`curso${cursoIndex + 1}`].notas
      : {};
    ...
    for (let j = 0; j < numAsignaturas; j++) {
      ...
      // Rellena con las notas existentes si están disponibles para no tener que volver a escribirlas
  ...
  function guardarNotasCurso(cursoIndex) {
    const notasDiv = asignaturasInputs
      .querySelectorAll(".curso-section")
      [cursoIndex].querySelector(".notas-asignaturas");
  ...
  function generarGrafico(data) {
    ...
    if (window.innerWidth < 768) {
      labels = ["Más reciente", "2º", "3º", "Más antiguo"];
    } else {
      labels = ["Curso más antiguo", "2º", "3º", "Curso más reciente"];
    }
    ...
    let sumaNotas = 0;
    const numCursos = data.values.length;
    ...
    data.values.forEach((notaMedia, index) => {
      ...
      li.style.setProperty(
        "--color-cuadradito",
        coloresPuntos[index % coloresPuntos.length]
      );
    ...
    const notaMediaTotal = sumaNotas / numCursos;

    const liTotal = document.createElement("li");
    liTotal.textContent = `\nNota media total: ${notaMediaTotal.toFixed(2)}`;
    //liTotal.style.fontWeight = 'bold';
    liTotal.style.setProperty("--color-cuadradito", "black");
    notasMediasLista.appendChild(liTotal);
    ...
    chart = new Chart(graficoRendimiento, {
      type: "line",
      data: {
        labels: labels.slice(0, data.values.length), // Etiquetas para el gráfico
        datasets: [
          {
